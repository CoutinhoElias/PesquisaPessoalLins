...
class BuscaCodigoProduto:
    ...
    def get_all_produtos(self):
        ...
        # with Session() força o fechamento da sessão
    ...
    def get_all_produtos_filtered(self):
        ...
        with Session() as session: 
            try:
                ...
                #     produto_dict['CdProdutoFornecedor'] = cd_produto_fornecedor[0].strip()
                # else:
                #     # Se não tiver nada na consulta ele seleciona o código do fornecedor mesmo
                #     produto_dict['CdProdutoFornecedor'] = f'erro - {self.cd_produto}'
                ...
                produto_dict.update({
                    'IdProduto': '1',
                    'NmProduto': 'ESTE PRODUTO NÃO FOI ENCONTRADO - DESCUBRA!',
                    'IdUnidade': 'UN',
                    'IdClassificacaoFiscal': '1',
                    'IdFornecedor': '1',
                    'CdFornecedor': '1',
                    'NmFornecedor': '1',
                    'CpfCgcFornecedor': '1',
                    'PrecoVendaAnterior': 0.0
                })
                return produto_dict
            # finally:
            #     session.close()  # Fecha a sessão
