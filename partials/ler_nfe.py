...
class LerNFE:
    def __init__(self):
        self.itens = []
        self.unificado = self.verifica_existencia_configuracao()

        self.produtos_informatica = {
    ...
    def verifica_existencia_configuracao(self):
        try:
            ...
            existing_data = self.unificado_padrao
    ...
    def informatica(self, ncm):
        if (ncm in self.produtos_informatica.keys()) or (ncm[0:4] in self.produtos_informatica.keys()) or (ncm[0:7] in self.produtos_informatica.keys()):
            return 'S', 'fatores_informatica'
        else:
            return 'N', 'default'

    def encontrar_regiao(self, estado):
        for regiao, estados in self.unificado["regioes_estados"].items():
            if estado in estados:
                return regiao
        return "Estado não encontrado."

    def dicionario_item(self, posicao, id_produto, quantidade, preco_unitario, ):
    ...
    def formatar_documento(self, numero):
    ...
    def get_text_or_default(self, element, tag, namespaces, default=None):
        tag_element = element.find(tag, namespaces)
        return tag_element.text if tag_element is not None else default

    def encontrar_valor(self, data, regiao, icms, cfop, origem, crt_emit=None, i=None):
        """
        ...
        if cfop in('5405', '6405', '5403', '6403'):
            return 0.0

        # Caso o ICMS seja 4, verifica diretamente o valor correspondente
        if float(icms) == 4.0:
            if 'default' in fatores and '4' in fatores['default']:
                return fatores['default']['4'] 
        ...
        # print(json.dumps(fatores, indent=4))
        # print(fatores['default'], '<<<-----------------')
        ...
        if 'default' in fatores and 'default' in fatores['default']: 
            ...
            return fatores['default']
        else:
            return fatores['default']['12']
    ...
    def ler_nfe(self, caminho_arquivo):
        ...
        try:
            ...
            regiao_emit = self.encontrar_regiao(emit.find('ns0:enderEmit/ns0:UF', namespaces).text if emit.find('ns0:enderEmit/ns0:UF', namespaces) is not None else "Endereço não encontrado")
            ...
            cpf_cnpj_fornecedor = self.formatar_documento(emissor['cnpj'])  # Formatar o CNPJ do emissor
            emissor['cnpj'] = cpf_cnpj_fornecedor

            cpf_cnpj_destinatario = self.formatar_documento(destinatario['cnpj_cpf'])  # Formatar o CNPJ do emissor
            ...
            for det in root.findall('.//ns0:det', namespaces):
                ...
                is_informatica, tipo_produto = self.informatica(ncm)
                cfop =  prod.find('ns0:CFOP', namespaces).text if prod.find('ns0:CFOP', namespaces) is not None else "CFOP não encontrado"
                ...
                produto = {
                    ...
                    'descricao': dicionario_produto_fornecedor['NmProduto'] if dicionario_produto_fornecedor['NmProduto'] is not None else 'Sem descrição disponível',
                ...
                if icms00 is not None:
                    ...
                    produto['impostos']['ICMS'] = {
                        ...
                        'aliquota': icms_item,
                        'aliquota_calculada': self.encontrar_valor(self.unificado, regiao_emit, icms_item, cfop, int(origem), crt_emit), # E
                ...
                # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                # ICMS60
                icms60 = imposto.find('.//ns0:ICMS/ns0:ICMS60', namespaces)
                if icms60 is not None:
                    origem = icms60.find('ns0:orig', namespaces).text if icms60.find('ns0:orig', namespaces) is not None else "Origem não encontrada"
                    produto['impostos']['ICMS'] = {
                        'origem': origem,
                        'CST': icms60.find('ns0:CST', namespaces).text if icms60.find('ns0:CST', namespaces) is not None else "CST não encontrado",
                        'base_calculo': icms60.find('ns0:vBCSTRet', namespaces).text if icms60.find('ns0:vBCSTRet', namespaces) is not None else "0",
                        'aliquota': icms60.find('ns0:pST', namespaces).text if icms60.find('ns0:pST', namespaces) is not None else "0",
                        'aliquota_calculada': 0,  # Para CST 60, geralmente é 0 pois já foi recolhido
                        'valor': icms60.find('ns0:vICMSSTRet', namespaces).text if icms60.find('ns0:vICMSSTRet', namespaces) is not None else "0"
                    }             
                # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------            
                # IPI
                ...
                produto['Custos'] = float(produto['impostos']['IPI']['pIPI']) + float(produto['impostos']['ICMS']['aliquota_calculada']) + float(self.unificado["custo"]["ir_fat"]) + float(produto['p_frete']) + 100 # 100 Pode ser transformado em percentual aqui?
                produto['MargemLucroAtual'] = float(produto['PrecoVendaAnterior'] * 100) / (float(produto['valor_unitario']) * (float(produto['Custos'])/100) * (float(self.unificado["custo"]["ir_fat"])/100)) # 100 Pode ser transformado em percentual aqui?
                produto['PrecoVendaCalculado'] = (float(produto['valor_unitario']) * (float(produto['Custos'])/100) * (float(self.unificado["custo"]["margem_lucro_esperado"])/100) * (float(self.unificado["custo"]["finan"])/100)) # 100 Pode ser transformado em percentual aqui?
                ...
                item = self.dicionario_item(posicao, produto['IdProduto'], float(produto['quantidade']), float(produto['valor_unitario']))
